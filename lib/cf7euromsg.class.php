...
class CF7EuroMsg{
  ...
  function adminAssets(){
    ...
    wp_enqueue_script( 'cf7-euromsg-notify-js', CF7EURMSGURL.'lib/js/notify.min.js', array(), '1.0.0', true );
  ...
  function saveSettings(){
    ...
    if(get_option("cf7euromsg_settings") === false){
      add_option("cf7euromsg_settings",$input["forms"]);
    }else{
      update_option("cf7euromsg_settings",$input["forms"]);
    }
  ...
  function login($username,$password){
		...
		$this->ticket = $wsLogin->Login((object)array("Username"=>$username,"Password"=>$password))->LoginResult->ServiceTicket;
		unset ($wsLogin);
    return $this->ticket;
	}
  ...
  function querySendLists($serviceTicket){
    $ws = new SoapClient(self::$serviceUrl . "sendlist.asmx?wsdl");
		$rv = $ws->QuerySendLists((object)array("ServiceTicket"=>$serviceTicket))->QuerySendListsResult->SendLists->EmSendListResponse;
		unset ($ws);
    return $rv;
  }
